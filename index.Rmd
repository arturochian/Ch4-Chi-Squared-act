...
`r take_home`) Use the mosaic plot to describe the cheating data.  Make sure that you note whether the two variables appear to be related.
...
`r take_home`) Why is this a parametric test?
...
```{r vote_dist_with_stat, warning=FALSE, cache=TRUE}
...
```{r cheat_dist, warning=FALSE, cache=TRUE}
B <- 1000
...
`r take_home`) What does the `shuffle` function do (ask if you are not sure)?  Explain how the `shuffle` function allows us to perform a permutation test.

---
# Take Home Question #`r take_home <- take_home + 1; take_home`

`r take_home`) Look back at the histograms.  Are any of the permuted test statistics anywhere close to as extreme as the observed test statistic?  Based on your answer, approximately what is the p-value?
...
`r take_home <- take_home + 1; take_home`) Write a conclusion *in the context of the problem* for the results of the Chi-Square test for the voting data.

`r take_home <- take_home + 1; take_home`) Take a look at the mosaic plot of the cheating data below.  
...
`r take_home <- take_home + 1; take_home`) What is the scope of inference for the Cheating Data?

`r take_home <- take_home + 1; take_home`) Why does random assignment of subjects to treatments allow us to make causal inferences?
